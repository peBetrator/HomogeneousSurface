...
void HalfConeSweep::draw(double R, double K, double r0, double r1) {
    ...
    auto makeStrip = [&](const Vec3& P, const Vec3& T, double rad, Vec3* out) {
        Vec3 w = T;
        double wl = std::sqrt(w.x * w.x + w.y * w.y + w.z * w.z);
        w = { w.x / wl, w.y / wl, w.z / wl };
        Vec3 base{ 0,0,1 };
        if (std::fabs(w.z) > 0.9) base = { 1,0,0 };
        Vec3 u{ w.y * base.z - w.z * base.y,
                w.z * base.x - w.x * base.z,
                w.x * base.y - w.y * base.x };
        double ul = std::sqrt(u.x * u.x + u.y * u.y + u.z * u.z);
        u = { u.x / ul, u.y / ul, u.z / ul };
        Vec3 v{ w.y * u.z - w.z * u.y,
                w.z * u.x - w.x * u.z,
                w.x * u.y - w.y * u.x };
